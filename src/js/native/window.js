...
class Cammy {
    ...
    this.window.on("close", (e) => {
      ...
      if (!this.saved) {
        var choice = dialog.showMessageBoxSync(this.window, {
          ...
          message:
            "Your note has unsaved changes. Do you want to discard them? ",
        });
  ...
  #setListeners() {
    ...
    var saveHandler = (e, args) => {
      ...
      fs.writeFile(args.path, cont, (err) => {
        if (err) {
          return Cammy.err(
            "Something went wrong while saving your file. Please report an issue if this error persists."
          );
        }
      });
    };
  ...
  #retitle() {
    if (this) {
      this.window.setTitle(
        (this.saved ? "" : "• ") + this.name + " - Cammy Notepad"
      );
    } else {
      delete this;
    }
    ...
    this.window.webContents.on("context-menu", (event, params) => {
      ...
      menu.append(
        new MenuItem({
          ...
          click: () => {
            this.window.webContents.send("addToCalendar");
          },
        })
      );
      menu.popup();
  ...
  #loadContent() {
    if (this.filePath) {
      ...
      if (!cont)
        return Cammy.err(
          "Something went wrong while opening this file. Please report an issue if this error persists."
        );
        ...
        if (stats && stats.isFile()) {
          var choice = dialog.showMessageBoxSync(this.window, {
            ...
            message:
              'The file "' +
              path.basename(pat) +
              '" already exists. Are you sure you want to replace this file?',
          });
      ...
      submenu: [
        {
          label: "Website",
          click: () => {
            shell.openExternal("https://github.com/sripkunda/Cammy");
          },
        },
      ],
    },

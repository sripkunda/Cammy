const editorModes = require(path.join(
  __dirname,
  "js/editor/data/editorModes.json"
));
...
class CammyEditor {
  ...
  updateState(bAddToCalendar) {
    if (
      editorModes[CammyEditor.editorMode] &&
      editorModes[CammyEditor.editorMode].parseEvents
    )
      parse(bAddToCalendar == true ? true : false);
    this.line = this.editor.codemirror.getCursor().line;
    document.querySelectorAll(".CodeMirror-line").forEach((e, i) => {
      if (i != this.line) {
        e.querySelectorAll(".cm-formatting").forEach((el) => {
          el.style.display = "none";
        });
        e.querySelectorAll(".cm-url").forEach((el) => {
          el.style.display = "none";
        });
      } else {
        e.querySelectorAll(".cm-formatting").forEach((el) => {
  ...
  static setEditorMode(mode) {
    ...
    if (!editorModes[mode].parseEvents)
      for (
        var i = 0;
        i < document.querySelectorAll(".CodeMirror-line").length;
        i++
      )
        formatLine(i, "none");
  }

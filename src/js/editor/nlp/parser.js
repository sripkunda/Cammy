...
function parse(bAddToCalendar) {
  ...
  const line = document.querySelectorAll(".CodeMirror-code pre")[ln]
    ? document.querySelectorAll(".CodeMirror-code pre")[ln].textContent
    : document.querySelectorAll(".CodeMirror-code pre")[--ln].textContent;
...
function getActivity(doc) {
  doc.dates().delete();
  return (
    doc.match("#Verb * #Noun", { fuzzy: 0.6 }).text() ||
    doc.match("#Noun", { fuzzy: 1 }).text() ||
    doc.match("#Verb", { fuzzy: 1 }).text()
  ); // return the activity
}
...
function addToCalendar(matches) {
  ...
  matches["date"].forEach((date, i) => {
    if (date.repeat) {
      repeating = true;
      obj["dates"] = formatDate(
        date.repeat.generated[0].start + "/" + date.repeat.generated[0].end
      );
      ...
      if (!dateSet)
        obj["dates"] = formatDate(
          matches["date"][0].start + "/" + matches["date"][0].end
        );

      if (!dateSet && date.unit == "time") dateSet = true;
    }
  });

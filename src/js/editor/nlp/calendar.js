...
const calendarLinks = require(path.join(
  __dirname + "/js/editor/data/calendarLinks.json"
));
...
class CammyCalendarLink {
  ...
  constructor(type, dat) {
    ...
    Object.keys(format).forEach((f, i) => {
      let val = dat[f];
      if (format[f].required && !val)
        throw Error("Required calendar field is missing!");
